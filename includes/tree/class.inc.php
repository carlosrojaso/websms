...
class tree {
	...
	// constructor
	function tree($tab,$a_id_column,$a_text_column,$a_father_column,$images_dir,$mode="graphic") {
	...
	function nb_childs($id_node) {
		return count($this->get_fathers($id_node));
	}
	
	// get the level of the item $id_node
	function level_of($id_node) {
		$res=0;
		if ($id_node==-1)
			return -1;
		else {
			while ($this->linear_tab[$this->get_line_of($id_node)][$this->father_column]!=-1) {
				$res++;
	...
	function get_next_line_tree($l) {
		...
		if ($this->tree_tab[$l]["symbol"]!="plus")
			return $l+1;
		else // c'est q'il y a un plus -> sauter autant de ligne qu'il y a de fils cachés
			for ($i=$l+1;$i<$this->height();$i++)
				if ($this->tree_tab[$i]["level"]<=$this->tree_tab[$l]["level"]) // dés que le niveau est inférieur ou égal
					return $i;
		return $this->height();
	}
